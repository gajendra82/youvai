...
class PrettyRatioPainter extends CustomPainter {
  ...
  void _pill(Canvas canvas, Size size,
      {required String your, String? golden, double scale = 1.0}) {
    final title =
        _tp(your, fs: 17 * scale.clamp(1.0, 1.3), fw: FontWeight.w900);
    ...
    final r = RRect.fromRectAndRadius(
      Rect.fromCenter(
          center: Offset(size.width / 2, 34 * scale), width: w, height: h),
      Radius.circular(24 * scale),
    );
    canvas.drawRRect(r, Paint()..color = _pillBg);

    final topLeft = Offset(size.width / 2 - title.width / 2,
        (34 * scale) - (h / 2) + (10 * scale));
    title.paint(canvas, topLeft);

    if (sub != null) {
      final subTop = topLeft +
  ...
  void _dashedH(Canvas c, double x1, double x2, double y,
      {required Paint p, double dash = 8, double gap = 5, double scale = 1.0}) {
    double x = x1;
    final d = dash * scale;
    final g = gap * scale;
    while (x < x2) {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final s = (size.shortestSide / 360.0)
        .clamp(1.1, 1.8); // bump slightly even on small screens
    ...
    if (mode == RatioMode.vertical) {
      ...
      for (int i = 0; i < 5 && i + 1 < xs.length; i++) {
        ...
        final rr =
            _bubbleAt(Offset((left + right) / 2, baseY + 16 * s), txt, s);
    ...
    if (mode == RatioMode.horizontal) {
      ...
      void h(double y, bool isEdge) {
        canvas.drawLine(
            Offset(leftBound, y), Offset(rightBound, y), isEdge ? edge : line);
      }
...
// ================= EYES =================
    ...
    if (mode == RatioMode.faceBox) {
      ...
      _dashedV(canvas, vx, rect.top + 14 * s, rect.bottom - 14 * s,
          p: dash, scale: s);
      _arrowUp(canvas, Offset(vx, rect.top + 14 * s), dash, s);
      _arrowDown(canvas, Offset(vx, rect.bottom - 14 * s), dash, s);

      final vTxt =
          _tp(hVal == 0 ? fb.yours : hVal.toStringAsFixed(3), fs: 13 * s);
      ...
      _dashedH(canvas, rect.left + 14 * s, rect.right - 14 * s, by,
          p: dash, scale: s);
    ...
    if (mode == RatioMode.noseLipChin) {
      ...
      final tagRR =
          _bubbleAt(Offset(guideX + 18 * s, top + (mid - top) * .15), tag, s);
    ...
    if (mode == RatioMode.lips) {
      ...
      if (lips.length >= 3) {
        ...
        _dashedV(canvas, rulerX, yTop, yBot,
            p: dash, dash: 6, gap: 4, scale: s);
        ...
        final ratioRR =
            _bubbleAt(Offset(rulerX + 30 * s, (yTop + yBot) / 2), ratio, s);
        canvas.drawRRect(ratioRR, _bubblePaint(ratioRR));
        ratio.paint(
          canvas,
